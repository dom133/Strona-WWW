		...
		if (is_uploaded_file($_FILES['plik']['tmp_name'])) {
			if ($_FILES['plik']['size'] > $max_rozmiar) {
				...
				move_uploaded_file($_FILES['plik']['tmp_name'],
						$_SERVER['DOCUMENT_ROOT'].'/avatars/'.$login.".jpg");
				$avatar = "http://www.skymin.pl/avatars/".$login.".jpg";
				mysqli_query($conn,"UPDATE `autoryzacja` SET `avatar`='$avatar' WHERE `User`='$login'");		
