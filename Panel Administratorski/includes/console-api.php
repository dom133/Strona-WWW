<?php
define( 'MQ_SERVER_ADDR', 'skymin.maxcraft.pl' ); //ip serwera minecraft
define( 'MQ_SERVER_PORT', 26113 ); //RCon port serwera minecraft
define( 'MQ_SERVER_PASS', 'Pawel1'); //haslo rcon serwera minecraft
define( 'MQ_TIMEOUT', 2 );
...
class MinecraftRcon
{
	...
	{
		$ftp_host = 'skymin.maxcraft.pl';
		$ftp_user = 'user';
		$ftp_pass = 'Pawel1';
		
		// Otwieranie połącznia
		$fp = fsockopen($ftp_host, 21, $err_no, $err_str, 30) or die($err_str);
		fread($fp, 4096);
		
		// Autoryzacja
		fwrite($fp, 'USER '.$ftp_user."\r\n");
		fread($fp, 4096);
		fwrite($fp, 'PASS '.$ftp_pass."\r\n");
		fread($fp, 4096);
		
		// Wysyłamy komendę ftp:
		fwrite($fp, 'PWD'."\r\n");
		// i odczytujemy
		return fread($fp+"/logs/latest.log", 4096);
